...
export default function Girl2460({ data }: ChartProps) {
  ...
  const getPenjelasanOtomatis = () => {
    ...
    if (refTerakhir) {
      if (terakhir.berat < refTerakhir.minus3) {
        ...
        saran = "Perlu konseling gizi, tingkatkan asupan makanan kaya protein hewani dan karbohidrat.";
        ...
        saran = "Batasi makanan/minuman manis serta jajanan tinggi lemak, dan tingkatkan aktivitas fisik anak.";
      }
    }
      ...
      <div className="w-full overflow-auto max-h-[500px] border border-gray-100 rounded-xl bg-gray-50/50 p-1">
        ...
        >
          <svg viewBox={viewBox} className="w-full h-auto select-none block">
            ...
            {anakData.length > 0 && (
              <path d={anakPath} fill="none" stroke="#e91e63" strokeWidth={2.5} />
            )}
            {anakData.map((d, i) => {
              ...
              return (
                <g key={`pt-${i}`}>
                  <circle cx={x} cy={y} r={6} fill="#ffffff" stroke="#e91e63" strokeWidth={1.5} />
                  <circle
                    cx={x}
                    cy={y}
                    r={4}
