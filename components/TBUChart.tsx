...
export default function TBUChart({ data, jenisKelamin }: TBUChartProps) {
  ...
  // ==========================================
  // LOGIKA PENJELASAN OTOMATIS (INTERPRETASI TB/U)
  // ==========================================
  const getPenjelasanOtomatis = () => {
    ...
    if (refTerakhir) {
      if (terakhir.tinggi < refTerakhir.minus3) {
        statusText = "Sangat Pendek";
      ...
      } else if (terakhir.tinggi < refTerakhir.minus2) {
        statusText = "Pendek (Stunted)";
        statusBg = "bg-amber-100 text-amber-800 border-amber-300";
        saran = "Perhatikan kecukupan gizi seimbang, terutama konsumsi protein hewani (telur, ikan, daging) dan kalsium.";
      } else if (terakhir.tinggi <= refTerakhir.plus3) {
        statusText = "Tinggi Badan Normal";
        statusBg = "bg-emerald-100 text-emerald-800 border-emerald-300";
        saran = "Pertahankan konsumsi gizi seimbang dan motivasi anak untuk aktif bergerak.";
      } else {
        statusText = "Tinggi";
        statusBg = "bg-blue-100 text-blue-800 border-blue-300";
        saran = "Pertumbuhan tinggi anak berada di atas rata-rata kelompok usianya. Lanjutkan pola hidup sehat.";
      }
    }

    // Evaluasi Tren Pertumbuhan Dibanding Bulan Sebelumnya
    if (sebelum) {
      const selisih = Number((terakhir.tinggi - sebelum.tinggi).toFixed(1));
      if (selisih > 0) {
        evaluasiTren = `Tinggi badan bertambah +${selisih} cm dari pengukuran sebelumnya (${sebelum.umur} bulan).`;
      } else if (selisih === 0) {
        evaluasiTren = `Tinggi badan tetap (${terakhir.tinggi} cm) dibandingkan bulan sebelumnya (${sebelum.umur} bulan).`;
      } else {
        evaluasiTren = `Tercatat penurunan tinggi badan ${selisih} cm dari bulan sebelumnya. Pastikan ketepatan alat ukur (microtoise).`;
      }
    } else {
      evaluasiTren = "Ini merupakan data pengukuran tinggi badan pertama yang tercatat pada rentang usia ini.";
    }
  ...
  return (
    <div className={`w-full bg-white rounded-2xl border ${jenisKelamin === "Laki-laki" ? "border-blue-200" : "border-pink-200"} p-5 shadow-sm space-y-4`}>
      ...
      <div className="flex items-center justify-between gap-2">
        <div>
          <h2 className="font-extrabold text-base md:text-lg text-gray-900 mb-0.5">
            Grafik Tinggi Badan Menurut Umur (TB/U)
          </h2>
              ...
              return (
                <g key={`pt-${i}`}>
                  ...
                  <circle
                    ...
                    onMouseEnter={() =>
                      setHover({ x, y, tanggal: d.tanggal, tinggi: d.tinggi, umur: d.umur })
                    }
      ...
      {/* KOTAK PENJELASAN OTOMATIS (ANALISIS TB/U) */}
      {penjelasan ? (
        <div className={`mt-4 p-4 rounded-xl border text-xs text-gray-700 space-y-2 ${jenisKelamin === "Laki-laki" ? "bg-slate-50 border-gray-200" : "bg-pink-50/50 border-pink-200"}`}>
          ...
          <div className="space-y-1 pt-1">
            <p>
              ...
              {penjelasan.terakhir.tinggi} cm pada umur {penjelasan.terakhir.umur} bulan
        ...
        <div className="mt-4 p-3 rounded-xl bg-gray-50 border border-dashed border-gray-300 text-center text-xs text-gray-500">
          Belum ada data pengukuran tinggi badan pada usia 24–60 bulan untuk menampilkan penjelasannya.
        </div>
      )}
    </div>
  );
